...
test("a click can step a real number input's own value up/down, for a quantity stepper, in a real browser", async () => {
  const compiled = compilePageSource(`Add a number input called quantity
...
When the decrease is clicked, subtract 1 from the value of quantity`);
  ...
  try {
    ...
    page.on("console", (msg) => { if (msg.type() === "error") errors.push(msg.text()); });
    await page.setContent(compiled.html);
    assert.equal(await page.locator("#element-1").inputValue(), "1");
    await page.locator("#element-2").click(); // increase: 1 -> 2
    await page.locator("#element-2").click(); // increase: 2 -> 3
    assert.equal(await page.locator("#element-1").inputValue(), "3");
    await page.locator("#element-3").click(); // decrease: 3 -> 2
    assert.equal(await page.locator("#element-1").inputValue(), "2");
    assert.deepEqual(errors, []); // no CSP violation, no runtime error
  } finally { await browser.close(); }
});
...
test("a click's if-conditional can check a real fixed range with \"is between ... and ...\" in a real browser", async () => {
  const compiled = compilePageSource(`Add a text input called score field
Add a paragraph called result
Set the text of result to none
Add a button called check
...
When the check is clicked, if the value of score field is between 1 and 10, set the text of result to valid otherwise set the text of result to out of range`);
  ...
  try {
    ...
    await page.locator("#element-1").fill("5");
    await page.locator("#element-3").click();
    assert.equal(await page.locator("#element-2").textContent(), "valid");
    await page.locator("#element-1").fill("1"); // inclusive of the low end
    await page.locator("#element-3").click();
    assert.equal(await page.locator("#element-2").textContent(), "valid");
    await page.locator("#element-1").fill("10"); // inclusive of the high end
    await page.locator("#element-3").click();
    assert.equal(await page.locator("#element-2").textContent(), "valid");
    await page.locator("#element-1").fill("11");
    await page.locator("#element-3").click();
    assert.equal(await page.locator("#element-2").textContent(), "out of range");
    assert.deepEqual(errors, []); // no CSP violation, no runtime error
  } finally { await browser.close(); }
});
...
test("a click can disable or enable a real form control in a real browser", async () => {
  const compiled = compilePageSource(`Add a checkbox called agree
Add a button called submit
Set the text of submit to Submit
...
When the toggle agree is clicked, if agree is checked, disable submit otherwise enable submit`);
  ...
  try {
    ...
    page.on("console", (msg) => { if (msg.type() === "error") errors.push(msg.text()); });
    await page.setContent(compiled.html);
    assert.equal(await page.locator("#element-2").isEnabled(), true);
    await page.locator("#element-1").check(); // a real user click on the checkbox
    await page.locator("#element-3").click(); // "toggle agree" -- re-evaluates the if
    assert.equal(await page.locator("#element-2").isDisabled(), true);
    await page.locator("#element-1").uncheck();
    await page.locator("#element-3").click();
    assert.equal(await page.locator("#element-2").isEnabled(), true);
    assert.deepEqual(errors, []); // no CSP violation, no runtime error
